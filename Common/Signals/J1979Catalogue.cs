...
public static class J1979Catalogue
{
    ...
    {
        // $00-$1F
        new(0x01, 4, "清除DTC后的监控状态", EncodeMonitorStatusSinceCleared, DecodeMonitorStatus),
        new(0x03, 2, "燃油系统状态",                EncodeFuelSystemStatus,          DecodeFuelSystemStatus),
        AnalogPid(0x04, 1, "计算发动机负荷",            SignalId.EngineLoad, PercentScalar, 0, "%"),
        AnalogPid(0x05, 1, "发动机冷却液温度",        SignalId.CoolantTemp, 1, -40, "degC"),
        AnalogPid(0x06, 1, "短期燃油修正B1",           SignalId.ShortTermFuelTrimBank1, TrimScalar, -100, "%"),
        AnalogPid(0x07, 1, "长期燃油修正B1",            SignalId.LongTermFuelTrimBank1, TrimScalar, -100, "%"),
        AnalogPid(0x08, 1, "短期燃油修正B2",           SignalId.ShortTermFuelTrimBank2, TrimScalar, -100, "%"),
        AnalogPid(0x09, 1, "长期燃油修正B2",            SignalId.LongTermFuelTrimBank2, TrimScalar, -100, "%"),
        AnalogPid(0x0A, 1, "燃油压力",                     SignalId.FuelPressure, 3, 0, "kPa"),
        AnalogPid(0x0B, 1, "进气歧管绝对压力", SignalId.ManifoldAbsolutePressure, 1, 0, "kPa"),
        AnalogPid(0x0C, 2, "发动机转速",                        SignalId.EngineRpm, 0.25, 0, "rpm"),
        AnalogPid(0x0D, 1, "车速",                     SignalId.VehicleSpeed, 1, 0, "km/h"),
        AnalogPid(0x0E, 1, "点火提前角",                    SignalId.TimingAdvance, 0.5, -64, "deg"),
        AnalogPid(0x0F, 1, "进气温度",            SignalId.IntakeAirTemp, 1, -40, "degC"),
        AnalogPid(0x10, 2, "空气质量流量",                     SignalId.MassAirFlow, 0.01, 0, "g/s"),
        AnalogPid(0x11, 1, "节气门位置",                 SignalId.ThrottlePosition, PercentScalar, 0, "%"),
        new(0x13, 1, "氧传感器存在（2列）", (_, s, _, d) => d[0] = s.O2SensorsPresent, DecodeO2Present),
        new(0x14, 2, "氧传感器B1S1", EncodeO2(SignalId.O2VoltageBank1Sensor1, SignalId.ShortTermFuelTrimBank1), DecodeO2),
        new(0x18, 2, "氧传感器B2S1", EncodeO2(SignalId.O2VoltageBank2Sensor1, SignalId.ShortTermFuelTrimBank2), DecodeO2),
        new(0x1C, 1, "OBD标准符合性", (_, s, _, d) => d[0] = s.ObdStandard, DecodeObdStandard),
        new(0x1F, 2, "发动机启动后运行时间", EncodeRunTime, data => $"{ReadU16(data)} s"),
        // $20-$3F
        new(0x21, 2, "MIL亮灯行驶距离", (_, s, _, d) => WriteU16(d, (ushort)(s.MilOn ? 12 : 0)), data => $"{ReadU16(data)} km"),
        AnalogPid(0x2F, 1, "燃油箱液位", SignalId.FuelLevel, PercentScalar, 0, "%"),
        new(0x30, 1, "清除故障码后暖机次数", (_, _, _, d) => d[0] = 40, data => data[0].ToString()),
        new(0x31, 2, "清除故障码后行驶距离", (_, _, _, d) => WriteU16(d, 1234), data => $"{ReadU16(data)} km"),
        AnalogPid(0x33, 1, "大气压力", SignalId.BarometricPressure, 1, 0, "kPa"),
        // $40-$5F
        new(0x41, 4, "本驾驶循环监控状态", EncodeMonitorStatusThisCycle, _ => "monitors complete"),
        AnalogPid(0x42, 2, "控制模块电压", SignalId.ControlModuleVoltage, 0.001, 0, "V"),
        AnalogPid(0x44, 2, "指令当量比", SignalId.CommandedEquivalenceRatio, 1.0 / 32768.0, 0, "lambda"),
        AnalogPid(0x45, 1, "相对节气门位置", SignalId.ThrottlePosition, PercentScalar, 0, "%"),
        AnalogPid(0x46, 1, "环境空气温度", SignalId.AmbientAirTemp, 1, -40, "degC"),
        ...
        AnalogPid(0x49, 1, "加速踏板位置D", SignalId.AcceleratorPedalPosition, PercentScalar, 0, "%"),
        AnalogPid(0x4A, 1, "加速踏板位置E", SignalId.AcceleratorPedalPosition, PercentScalar, 0, "%"),
        new(0x51, 1, "燃油类型", (_, s, _, d) => d[0] = s.FuelType, DecodeFuelType),
        AnalogPid(0x5C, 1, "发动机机油温度", SignalId.EngineOilTemp, 1, -40, "degC"),
    };
