...
* **Per-ECU protocol-stack bindings** (`Core/Protocol/`). Each ECU resolves an inbound `(CAN id, SID)` to a stack binding and dispatches there: a GM ECU binds J1979 + GMW3110 (`Gmw3110Dispatch`); after a successful `$36` sub `$80` DownloadAndExecute a transient UDS (ISO 14229) kernel binding (`UdsKernelDispatch`) takes over, presenting the services a real GM SPS kernel exposes (`$31` RoutineControl - EraseMemory, CheckMemoryByAddress, finalisation - plus `$3E` / `$20` / second-stage `$34`/`$36`), and is torn down on `$20` or P3C timeout. A Ford ECU binds a single catch-all UDS capture stack (`FordUdsDispatch`) for replaying captured Ford-stack traffic. Per-binding service allow-lists are editable in the ECU's Advanced tab and persisted as a delta (`EcuDto.Stacks`).
